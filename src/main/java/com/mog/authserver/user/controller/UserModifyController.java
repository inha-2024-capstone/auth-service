...
public class UserModifyController {
    ...
    public ResponseEntity<BaseResponseBody<Void>> modifyPassword(
            @AuthenticationPrincipal AuthenticatedUserInfo authenticatedUserInfo, @RequestBody String password) {
        userInfoModifyService.modifyPassword(authenticatedUserInfo.id(), password);
    ...
    public ResponseEntity<BaseResponseBody<Boolean>> validatePassword(
            @AuthenticationPrincipal AuthenticatedUserInfo authenticatedUserInfo, @RequestBody String password) {
        Boolean isSamePassword = userInfoValidationService.validateSamePassword(authenticatedUserInfo.id(), password);
        return ResponseEntity.status(SuccessStatus.OK.getHttpStatus())
                .body(SuccessStatus.OK.getBaseResponseBody(isSamePassword));
    }

    @PostMapping("/nickname")
    public ResponseEntity<BaseResponseBody<Boolean>> validateNickname(@RequestBody String nickname) {
        Boolean isSamePassword = userInfoValidationService.doesNickNameExist(nickname);
        return ResponseEntity.status(SuccessStatus.OK.getHttpStatus())
                .body(SuccessStatus.OK.getBaseResponseBody(isSamePassword));
    }
    ...
    public ResponseEntity<BaseResponseBody<Void>> modifyNickname(
            @AuthenticationPrincipal AuthenticatedUserInfo authenticatedUserInfo, @RequestBody String nickname) {
        userInfoModifyService.modifyNickname(authenticatedUserInfo.id(), nickname);
        return ResponseEntity.status(SuccessStatus.OK.getHttpStatus()).body(SuccessStatus.OK.getBaseResponseBody());
    }

    @PostMapping("/email")
    public ResponseEntity<BaseResponseBody<Boolean>> validateEmail(@RequestBody String email) {
        Boolean isSameEmail = userInfoValidationService.validateDuplicateEmail(email);
        return ResponseEntity.status(SuccessStatus.OK.getHttpStatus())
